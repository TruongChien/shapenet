...
import external.graphrnn_eval.stats
import utils
# import main.Args

# from shaper.GraphRNN.model_config import ModelSpecs
# from shaper.GraphRNN.utils import perturb, load_generated_graphs
import logging
...
def load_ground_truth(dir_input, dataset_name, model_name='GraphRNN_RNN'):
    ...
    if model_name == 'Internal' or model_name == 'Noise' or model_name == 'B-A' or model_name == 'E-R':
        ...
        fname_test = dir_input + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
            hidden) + '_test_' + str(0) + '.dat'
    try:
        graph_test = load_generated_graphs(fname_test)
    except:
...
def eval_single_list(graphs, model_spec: ModelSpecs):
    """
    Evaluate a list of graphs by comparing with graphs in directory dir_input.
    """
    graph_test = model_spec.load_train_test()
...
def evaluation_epoch(dir_input,
                     ...
                     model_name,
                     dataset_name,
                     args, is_clean=True, epoch_start=1000,
...
def eval_performance(datadir, prefix=None, args=None, eval_every=200, out_file_prefix=None,
    ...
    if args is None:
        ...
        real_graph_filename = datadir + args.dir_graph_save + args.fname_test + '0.dat'
        # for proposed model
        end_epoch = 3001
        epoch_range = range(eval_every, end_epoch, eval_every)
        pred_graphs_filename = [
            datadir + args.dir_graph_save + args.fname_pred + str(epoch) + '_' + str(sample_time) + '.dat'
