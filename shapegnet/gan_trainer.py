...
from shapenet.shapegnet.generator_trainer import GeneratorTrainer
from shapenet.shapegnet.model_config import ModelSpecs


class Trainer(GeneratorTrainer):
    def __init__(self, trainer_spec: ModelSpecs,
                 graph, models: dict,
                 graph_weighted, scaler, N, max_iterations=20000, rw_len=16, batch_size=128, H_gen=40,
                 H_disc=30, H_inp=128, z_dim=16, lr=0.0003, n_critic=3, gp_weight=10.0, betas=(.5, .9),
                 l2_penalty_disc=5e-5, l2_penalty_gen=1e-7, temp_start=5.0, temp_decay=1 - 5e-5, min_temp=0.5,
                 val_share=0.1, test_share=0.05, seed=498164, delta=0.02):
        """
            Initialize NetGAN.
            Parameters
            ----------
        ...
        if 'generator' not in models:
            raise Exception("model require generator.")
        self.generator = models['generator']

        if 'generator' not in models:
            raise Exception("model require discriminator.")
        self.discriminator = models['discriminator']

        self.generator.freeze_params('structure')
        self.G_optimizer = optim.Adam(filter(lambda p: p.requires_grad,
        ...
        self.scaler = scaler
        self.train_ones, self.val_ones, self.val_zeros, self.test_ones, self.test_zeros = utils.train_val_test_split_adjacency(
            graph, val_share, test_share, seed, undirected=True, connected=True, asserts=True)
        self.train_graph = sp.coo_matrix(
        ...
        self.walker = utils.RandomWalker(self.train_graph, self.graph_weighted, self.rw_len, p=1, q=1,
    ...
    def calc_gp(self, fake_inputs, real_inputs):
        ...
        gradients = \
            grad(outputs=y_pred_interpolates, inputs=interpolates, grad_outputs=torch.ones_like(y_pred_interpolates),
    ...
    def critic_train_iteration(self):
        ...
        real_inputs = one_hot(torch.tensor(random_walks, dtype=torch.int64), num_classes=self.N).type(torch.float64).to(
            self.device)
        ...
        disc_cost = torch.mean(y_pred_fake) - torch.mean(y_pred_real) + gp + self.l2_regularization_D(
            self.discriminator)
    ...
    def generator_train_iteration(self):
        ...
        if self.state == 'structure':
            ...
            gen_cost = -torch.mean(y_pred_fake) + self.l2_regularization_G_lines(self.generator)
        gen_cost.backward()
        ...
        if self.state == 'structure':
            if self.stopping_criterion == 'val':
                ...
                if self.patience == 0:
                    ...
                    self.G_optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.generator.parameters()),
    ...
    def train(self, create_graph_every=2000, plot_graph_every=500, num_samples_graph=100000, stopping_criterion='val',
              max_patience=5, stopping_eo=None, i_start=0):
        """
